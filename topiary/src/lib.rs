...
pub fn formatter(
    ...
    match operation {
        Operation::Format {
            ...
            let mut atoms =
                tree_sitter::apply_query(&content, query, grammars, tolerate_parse_errors, false)?;
            ...
            if !skip_idempotence {
                idempotence_check(&trimmed, query, language, grammars, tolerate_parse_errors)?;
            }
        ...
        Operation::Visualise { output_format } => {
            let (tree, _) = tree_sitter::parse(&content, grammars, false)?;
